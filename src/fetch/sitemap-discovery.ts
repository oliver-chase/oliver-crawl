...
export async function discoverSitemapUrls(
  ...
  try {
    ...
    return { urls: [], truncated: false, reason: error instanceof Error ? error.message : String(error) };
  }
  ...
  if (!xml) return { urls: [], truncated: false, reason: 'No sitemap.xml (or it did not answer 200).' };
  ...
  if (isSitemapIndex(xml)) {
    ...
    for (const child of children) {
      try {
        ...
        if (childXml && !isSitemapIndex(childXml)) pageLocs.push(...extractLocs(childXml));
      } catch {
        // one bad child sitemap doesn't spoil the rest
      }
      if (pageLocs.length >= maxUrls + 1) break;
    }
  } else {
    pageLocs = extractLocs(xml);
  }
  ...
  for (const loc of pageLocs) {
    ...
    try {
      const safe = assertRequestUrlAllowed(target, loc).toString();
      if (!seen.has(safe)) {
        seen.add(safe);
        urls.push(safe);
      }
  ...
  return {
    urls,
    truncated: pageLocs.length > urls.length && urls.length === maxUrls,
