...
| `GUARD-PRECISION-4` | The tool-exfiltration destination may be an EMAIL ADDRESS, not only a bare host. `to attacker@evil.com` was missed because the destination clause cannot match across an `@`, so the narrowing that fixed two live false positives had dropped a case the extractor this descends from still catches — and whose own test asserts it. | `guard/prompt-injection-guard.ts` | `guard/guard-precision.test.ts` |
| `GUARD-PRECISION-5` | `encoded-payload` required only an 80-character alphanumeric run, which whitespace-stripped page copy produces constantly — a nav mega-menu flattening to `businessmarketingoperations...` quarantined a whole page, about 8% of a mainstream B2B sample. Base64 of any real length carries mixed case AND digits, or explicit `=` padding; concatenated English words carry neither. | `guard/prompt-injection-guard.ts` | `guard/guard-precision.test.ts` |
| `TIMEOUT-JINA-1` | The Jina rung ignored the caller's `timeoutMs` and used its own 30s, so a crawl bounded at 8 seconds could still wait 30 on that rung. A caller cannot see which rung is running, so the one number they set has to hold on every rung. The 30s remains the default for callers who set nothing. | `fetch/jina-fetch.ts` | `fetch/jina-timeout.test.ts` |
| `QUARANTINE-EVIDENCE-1` | A quarantine refusal carries the signals that tripped it and the SANITIZED text, so a consumer whose policy is "never lose a page" can raise a review task. It returned a bare refusal, indistinguishable from a fetch failure at the call site, which left such a consumer nothing to build from and forced a silent drop — the exact outcome quarantining exists to prevent. Text is sanitized, not raw: a reviewer needs to see what the page said, not be handed a live payload. | `fetch/build-page.ts` | `guard/quarantine-evidence.test.ts` |
| `ORIGIN-MOVED-1` | When the direct rung refuses an OFF-DOMAIN redirect and Jina serves the page anyway, the refusal travels with the result. Found on a live source: isisasheville.com was sold and now redirects to a Spanish medical centre, so the crawl returned that content as an ordinary success under the old URL. Refusing outright is the wrong fix — an off-domain redirect is routinely a real migration, and this fleet already reversed one over-strict redirect rule that cost six sources to stop one parked domain. | `lanes/own/index.ts` | `lanes/origin-moved.test.ts` |
| `RENDER-REDIRECT-2` | The REMOTE render rung validates where the service says it landed. RENDER-REDIRECT-1 hardened the local rung and the remote one had no equivalent anywhere: `data.url` came from the service and was trusted into `buildPage`, so a redirect off the requested site returned as ordinary content under the requested URL. Checked inside `renderViaService`, because a caller that forgets is the failure mode and that is the only place the answer enters. | `fetch/browser-render.ts` | `lanes/origin-moved.test.ts` |
| `BODY-RECEIVED-1` | A failure says whether an origin actually answered. `unreachable` covers both "the site is down" and "the site served a JavaScript shell with no readable text", because from the ladder's side both end with no content — but those are different facts downstream: the first is worth retrying, the second is a page that was read and had nothing in it, which a catalogue may RETAIN rather than log. The origin app was recovering this by regex-matching `detail`, the coupling that has silently switched several signals off in this fleet. | `lanes/own/index.ts` | `lanes/body-received.test.ts` |
| `GUARD-TELEMETRY-1` | Every page reports `redactionCount` and `truncated`. The sanitiser already computed both and they were discarded before the page was built, yet neither is recoverable afterwards: a capped page is one whose tail was never seen, and a non-zero redaction count means the text differs from what the origin served. Both change how far a downstream extraction should be trusted. | `fetch/build-page.ts` | `lanes/page-shape.test.ts` |
| `GUARD-REDACT-1` | Redaction follows detection: only patterns that raised a signal are redacted. The loop ran every pattern over un-stripped text while detection evaluates `encoded-payload` URL-free, so GUARD-PRECISION-3 removed the false quarantine and left the false REDACTION — an ordinary page had a working asset URL silently replaced mid-string in both `text` and `markdown`. It also restores the inference that a non-zero `redactionCount` means an injection was present. | `guard/prompt-injection-guard.ts` | `guard/guard-precision.test.ts` |
...
| `RENDER-SILENT-1` | The render rung swallowed every failure into null, so an active redirect attack was indistinguishable from playwright not being installed. Security refusals now emit a usage event; ordinary unavailability still does not. | `fetch/local-render.ts` | `fetch/render-wiring.test.ts` |
| `RENDER-SUBRESOURCE-1` | Page JavaScript could fetch a CORS-permissive service on a private address and land the body in the DOM, which then leaves in page.content(). Subresource requests are now resolution-checked. Same-site is deliberately NOT applied — real pages load public CDN assets. | `fetch/local-render.ts` | `fetch/render-wiring.test.ts` |
| `RENDER-HOP-1` | Every navigation hop is validated, not just the landing. Checking only the final URL let a chain via 127.0.0.1 render normally while the private server received a real request — the request is the leak, and a chain that returns home hides it. | `fetch/local-render.ts` | `fetch/render-wiring.test.ts` |
| `RENDER-HOP-2` | The render rung follows redirects ITSELF, with `maxRedirects: 0` per hop, instead of calling `route.continue()`. Chromium follows 3xx inside the network stack and never shows the route handler the hops, so RENDER-HOP-1's check was present, correct and never invoked — driving `site -> off-site -> site` showed ONE request reaching the handler while three responses arrived. The off-site server got a real request and the landing check passed because the chain came home. | `fetch/local-render.ts` | `fetch/render-hop-chain.test.ts` |
