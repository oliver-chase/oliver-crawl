...
| Date | Entry | Change |
|---|---|---|
| 2026-07-28 | gates on commit | The comment budget and the decision ledger are now pre-commit gates here, not just `npm run check` steps, so a commit that skips validation can no longer land an over-cap block or an unrecorded id. Both were clean when they went in: 76 ids in the source against 76 ledger rows, no block over the cap. Two upstream gate defects were fixed in the same pass — a ledger row shaped so no check could read it, and a user-story id (`US-*`) read as a decision record. |
